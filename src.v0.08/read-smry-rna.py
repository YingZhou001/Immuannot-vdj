...
def is_constant_gene(gene) :
    ...
    ret = gene.replace('"', '').split('*')[0] in c_genes
    return(ret)
...
for line in sys.stdin:
    ...
    if is_constant_gene(gene_name) :
        ...
        ele = attr['template_source'].split('=')[1].split('|')[0]
        mapping_rate = float(attr['mapping_rate'])
        gene_name = gene_name + '*' + ele
...
def convert_feature(feature, strand, gene):
    ...
    if feature in sym_map :
        ...
        ret = gene.split('*')[1].replace('-STOP-CODON', '$').replace('-REGION', '')
    else :
        ret = '.'
...
def cal_dis(x, y) :
    ...
    if x[1] < y[0] or y[1] < x[0] :
        dis = min(abs(y[0] - x[1]), abs(x[0] - y[1]))
    else :
        dis = 0
    return(dis)
...
for ctg in db:
    ...
    all_genes.append(old_gene.split('*')[0])
    ...
    for i in range(1,L) :
        ...
        dis = cal_dis([old_fro, old_to], [fro, to])
        g1 = old_gene
        g2 = gene
        f1 = old_tag.upper()
        f2 = tag.upper()
        ...
        is_ll = '^' in f1 and '^' in f2
        is_lv = '^' in f1 and 'V' in f2
        is_lv = is_lv or ('^' in f2 and 'v' in f1)
